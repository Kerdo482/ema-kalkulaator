...
function getinfo(){
  ...
  .then((data) => {
    ...
    //statDiv.innerHTML =  (palkNr * 1.645).toFixed(2) + "€ Tunnipalk"
    // palk * tunnid
    mathcalc = (palkNr * currenthrs) * nightmultipl * 1.645
    // total
    currentvalue = currentvalue + mathcalc
    totaldiv.innerHTML = currentvalue.toFixed(2)+ "€ Kokku"

    //history
    let newRow = document.createElement("div");
    newRow.classList.add("print-in");
    if (checkBox.checked == true){
      newRow.innerHTML = " " + jobname + " " + currenthrs + " öötundi  " + '<div class="price">' + mathcalc.toFixed(2) + "€ </div>";
    } else {
      newRow.innerHTML = " " + jobname + " " + currenthrs + " tundi  " + '<div class="price">' + mathcalc.toFixed(2) + "€ </div>";
    }
